...
public class PDFExportUtil {
    ...
    public static void export(List<ClassSchedule> schedules, String filePath) throws IOException {
        ...
        Files.write(Path.of(filePath), exportToBytes(schedules));
    }

    /** Creates the same class timetables in memory for the read-only download API. */
    public static byte[] exportToBytes(List<ClassSchedule> schedules) throws IOException {
        if (schedules == null || schedules.isEmpty()) return new byte[0];
