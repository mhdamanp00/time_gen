...
public class ScheduleService {
    ...
    private void checkConflicts(ClassSchedule schedule, Long excludeId) {
        ...
        if (dao.isClassBooked(schedule.getClassName(), day, period, excludeId)) {
            throw new ConflictException(
                "⚠ CLASS CONFLICT\n\nClass/section \"" + schedule.getClassName() +
                "\" already has a lesson on " + day.getDayName() + " — " + period +
                ".\n\nChoose a different day or period."
            );
        }
    }
}
