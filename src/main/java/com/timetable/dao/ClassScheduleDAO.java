...
public class ClassScheduleDAO extends GenericDAO<ClassSchedule> {
    ...
    /** Checks whether a class already has a lesson at the given day and period. */
    public boolean isClassBooked(String className, WorkingDay day, TimePeriod period, Long excludeId) {
        try (Session session = openSession()) {
            String hql = "SELECT COUNT(s) FROM ClassSchedule s " +
                    "WHERE LOWER(s.className) = :className AND s.workingDay = :day AND s.timePeriod = :period" +
                    (excludeId != null ? " AND s.id != :excludeId" : "");
            var q = session.createQuery(hql, Long.class)
                    .setParameter("className", className.trim().toLowerCase())
                    .setParameter("day", day)
                    .setParameter("period", period);
            if (excludeId != null) q.setParameter("excludeId", excludeId);
            return q.uniqueResult() > 0;
    ...
    /** Atomically replaces the timetable, or appends proposed rows when replace is false. */
    public void applyGenerated(List<ClassSchedule> schedules, boolean replace) {
        org.hibernate.Transaction tx = null;
        try (Session session = openSession()) {
            tx = session.beginTransaction();
            if (replace) session.createMutationQuery("DELETE FROM ClassSchedule").executeUpdate();
            for (ClassSchedule schedule : schedules) session.persist(schedule);
            tx.commit();
        } catch (Exception e) {
            if (tx != null) tx.rollback();
            throw new RuntimeException("Failed to apply generated timetable", e);
        }
    }
}
